class Solution:
    def findMin(self, nums: List[int]) -> int:
        ...
        # the logic seems identify as previous
        # I think the foundamental differnece is, when mid is the max, it will be in the first part, but it will be push to the second part as l+1
        # now, if the loop not end, the problem became we are searching on the secondpart
        # on the secondpart, the left based appraoch act the same, basically is pushing the mid to the right since l from the minimum of first part became the minimum of the second part, so think about mid >= l as always
        # however, the r based appraoch, in the first part, the mid always > r, so it push l to mid + 1
        # in the second part, the situation reverse, when the mid come to the second part, the r always > mid, then it can push it to the minimum
        while l < r:
            mid = (l + r)//2
            print(l,r,mid)
            if nums[mid] <= nums[r]:
